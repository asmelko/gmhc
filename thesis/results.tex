...
The results were compared with a different\todo{the existing?}\ implementation of Mahalanobis-average hierarchical clustering \cite{fivser2012detection}. \xxx{It} is a R language implementation with performance critical parts written in C language\todo{rovnou tam zmin ze tam maj BLAS}. It provides both FMD and CMD measures of dissimilarity and the apriori clusters computation as well. It performs a~serial CPU computation. For brevity, we denote the thesis GPU implementation by \texttt{gmhclust} and the serial CPU version by \texttt{mhclust}.
...
\begin{description}
	...
	\item[Apriori datasets] These datasets are clustered with apriori clusters\todo{Tohle je vhodny uplne prepsat, postupne 1-- Apriori datasety jsme pouzili uplne umely (alternativne by bylo mozny vzit normalni point datasety a predclusterovat je treba k-meansem) 2-- apriori clustery jsou v datasetech maly gaussovsky clustery, ty jsou kazdej oznackovnej jako jeden apriori, 3-- celej dataset obsahuje hodne techhle clusteru umistenejch na jinejch mistech}. They do not represent any measured data. Rather, for each dataset, a set of distinct clusters was artificially created for this purpose. Each cluster was randomly generated as a set of $d$-dimensional points with the normal distribution and evenly scattered among the dataset. The describing parameters of~apriori datasets \xxx{are the}\todo{comprise}\ \emph{apriori cluster size}, the \emph{apriori cluster count} and the \emph{point dimensionality} (see tab.~\ref{tab04:apriori}). To find the apriori clusters, they need not be pre-computed by another clustering algorithm (such as k-means) as~all the~points are already stored with respect to the apriori clusters.
...
All experiments were run on the same machine (Intel Xeon Silver 4110, 256 GB RAM, NVIDIA Tesla V100 PCIe 16 GB). The performance-related experiments were performed several times\todo{why and how many times}. As there were negligible performance differences in the separate runs\todo{tos fakt pozoroval na vsech experimentech? Jestli jo, rekni ze sis overil ze experimenty nemely zadny velky vykyvy a pouzivas jen aritmeticky prumery.}, all the following plots depicting performance show their arithmetic means. 
...
As \xxx{can be} seen from the figure \ref{fig04:neigh}, the variant of 1 closest neighbor achieved the~best performance. It\todo{asi spis `this'}\ may be caused by the fact that the lower number of cluster updates did not overcome the increased time for each update. Alternatively, it may be caused by the composition of tested datasets; datasets that naturally form cluster of other than elliptical shape may benefit better from another closest neighbor number variant.

The forthcoming experiments will be performed with the 1 closest neighbor variant.

\section{Performance speedup}
\todo{Presnejsi nadpis. Tadyto zni jako: Mame speedup! --- Co se zrychlilo? --- Performance! Podobne to chces i niz, `Speedup on apriori datasets'}

The \xxx{next} set of experiments determines the performance speedup of~the~GPU implementation over the CPU implementation. The performance comparison was performed on single-point and apriori datasets separately.
...
\begin{figure}\centering
	...
	\caption{An example of a \xxx{poor} performance of \texttt{mhclust} compared with \texttt{gmhclust} on single-point datasets. Datasets of \texttt{mhclust} were down-sampled to the fraction of $\frac{1}{10}$ while \texttt{gmhclust} datasets were not changed. For Samusik\_all dataset, neither the fraction of $\frac{1}{10}$ did help to finish the computation within a 24 hours mark. \xxx{Nikdy nerikej ze nekdo ma horsi vykon (vytocis tim oponenta kterej ho muze znat), misto toho proste ze ty mas lepsi vykon :D }}
...
In contrast \xxx{with} the single-point datasets, the CPU implementation managed to~compute apriori datasets of big sizes. It is a consequence of the wisely chosen apriori clusters.
...
To compare two clusterings, we compute the height of each point in both clusterings. Then we take the two distances of each point as its coordinates and put them on the two-dimensional plane\todo{tomuhle se rika scatterplot}. If they create a straight increasing line\todo{tohle chce obratit, kdyz korelujou, tak clustery jsou podobny a je to videt jako rovna cara}, it means that the correlation between point heights of two clusterings is equal to~1 and that clusterings formed very similar clusters. When points are scattered among the whole plane, different clusters were created and the overall clusterings differ.

\xxx{An another}\todo{alternative?}\ approach for comparing hierarchical clusterings is\todo{would be to}\ to compare \xxx{a selected parts} of their dendrograms. Dendrograms are \emph{cut} to expose a small amount of clusters (i.e.~10). Then, they are directly compared using similarity measures like the F-score.

However, \xxx{a} MHCA algorithm tends to create small clusters that are propagated to~the~very top of the dendrogram. They usually consist of noise; \todo{because?} Mahalanobis distance makes them dissimilar to the rest and they are merged in the late clustering stages. This can be a problem for the \xxx{direct cluster comparison}\todo{myslis konkretne porovnavani na cut-nutych clusterech? V tom pripade } as these noise clusters can easily skew the results. In compared clusterings, they may be composed of different points or form different amount of cluster; all this can worsen the result of potentially similar clusterings. 

Hence, we preferred the first method for comparing two clusterings as it takes the whole dendrogram into the account rather than a selected part.\todo{tahle veta asi neni potreba}
