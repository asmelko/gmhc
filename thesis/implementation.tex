...
\begin{figure}\centering
	...
	\caption{An example of 2D grid of size (3,2) with 2D blocks of size (4,3). \xxx{zdroj obrazku?}}
	\label{fig02:grid}
\end{figure}
...
\begin{figure}\centering
	...
	\caption{An example of different grid block distributions among SMs. \xxx{zdroj obrazku?}}
	\label{fig02:SM}
\end{figure}
...
\todo{Tady mas terminologii, ale vubec nikde neni zdurazneny proc to je vlastne rychly a jaky jsou hlavni metody jak to optimalizovat. Hod sem kratkou sekci `GPU performance and optimization concerns' kde to zhruba vysypes.}

\section{Implementation strategy}

The first chapter\todo{idealne cref primo na tu sekci}\ introduced three \xxx{views on} implementation of a hierarchical clustering analysis. To decide which one to choose, \xxx{we need to take into an account two conditions.} First, the algorithm \xxx{should not require a quadratic space complexity} as it must be able to process rather large datasets. Second, the algorithm should exhibit some parallelism opportunities; otherwise massive GPU parallel properties will be to no use.  We compare the mentioned algorithms according to these conditions:

\todo{mozna z toho chces udelat prehlednou tabulku, kde bude `memory used' a `independent threads' oddelene vyjadreny v zavislosti na $n$, $d$ a $k$.}

\begin{description}

\item[HCA with dissimilarity matrix] satisfies the \xxx{first condition}\todo{nikdo si nepamatuje ze si je ocisloval, ty podminky normalne pojmenuj}\ if the dissimilarity matrix \xxx{will not be} held in the memory. The second condition is satisfied as each matrix field can be computed independently, exposing great parallelism opportunity. 

\item[HCA with the nearest neighbor array] \xxx{has needed} sub-quadratic space complexity by definition. On the other hand, it provides less space for a parallelism as the algorithm \xxx{need not to} compute whole dissimilarity matrix as~in~the~previous algorithm. \xxx{However,} large dataset sizes may compensate for this disadvantage. 

\item[HCA with priority queues] is \xxx{unusable}\todo{nekdo by to urcite naimplementoval a slo by to, akorat by to bylo pomaly a nudny} as it does not satisfy the first condition. Moreover, operation over priority queues (insert, delete and min) are not of a parallel nature and may create a bottleneck in the computation.
...
Additionally, we chose to implement the CMD variant of the Mahalanobis distance to fully exhibit GPU parallel properties.\todo{to souvisi jak?}

\subsection{Apriori clustering optimization}

When cluster sizes reach the Mahalanobis threshold, a MHCA algorithm starts to~form elliptic clusters. However, until then, a simple euclidean distance is used. This is the important stage of the algorithm as an improper initial clustering can \xxx{decrease the overall clustering result}. Hence, a simple hierarchical euclidean clustering can be unsatisfactory.
\todo{Ta apriori optimizace je hlavne proto aby ses vyhnul tomu ze mas miliardu bodu a potrebujes pocitat miliardu na druhou vzdalenosti; to ze to pomaha na zacatku s tim ze }

To solve this problem, we \xxx{implemented an additional feature}\todo{tohle zni jako kdyz claimujes zes to vymyslel}\ called the \emph{apriori clusters}.\todo{eeeeeeeeeeeeemph!} 

\begin{defn}[Apriori clusters]\todo{Apriori clustering}
	\xxx{Suppose} a dataset $\mathcal{D}$, a HCA algorithm $\mathcal{A}$ and a~partitioning of  $\mathcal{D}$ into non-empty subsets $\mathcal{D}_1,\dots,\mathcal{D}_k$. \xxx{If following} conditions hold:
	\begin{enumerate}
...
The intended usage of this feature is that the apriori clusters are created from a dataset using non-hierarchical clustering algorithm such as \emph{k-means} --- an algorithm that is capable of creating \xxx{better initial clusters}\todo{to `better' neni pravda, dulezity je ze sou rychlejc. Ale rozhodne se tim vyhejbas ruznejm potizim se singularitou ty kovariantni matice}\ that the MHCA algorithm (see fig.~\ref{fig03:apr_ex}).
...
\begin{figure}\centering
	...
	\caption{An example dataset with (right) and without (left) visualized pre-computed apriori clusters (each apriori cluster is visualized as an ellipse --- dataset points contained in an ellipse represent a subset of the dataset).\xxx{Tohle vypada dost podezrele, skoro jako ze apriori clustery to prakticky vyresily :D }}
	\label{fig03:apr_ex}
\end{figure}

\section{Generic interface to clustering algorithms}
\todo{Nazev kapitoly doporucuju zmenit na `Benchmarking framework' nebo tak neco}

The chosen algorithm\todo{jen jeden? Taky by bylo vhodny trochu `switchnout' do toho ze implementujes, ze to kodis v C++ apod.} is implemented in the main class of the program --- \texttt{gmhc}\footnote{An abbreviation for GPU Mahalanobis-average Hierarchical Clustering}. It inherits from an abstract template class \texttt{hierarchical\_clustering}.
...
The class \texttt{gmhc} is the entry point of the whole \todo{gpu-accelerated?} MHCA algorithm. It inherits from \texttt{hierarchical\_clustering<float>} template specialization, which means that the algorithm expects dataset objects to be single-precision points of~a~specified dimension. The class communicates
...
Next follows the main responsibility of the \texttt{initialize} method --- device and host data allocation and initialization. They are \emph{point}, \emph{assignment}, \emph{centroid}, \emph{inverse covariance}, \emph{neighbor} and \emph{status arrays}. \todo{Tady je emph asi OK, ale rovnou sem napis ty zkracujici pismena.}
...
\begin{defn}[Cluster-based arrays]\todo{Cluster-based znamena `zalozeny na clusterech', chces spis `Per-cluster'?}
	...
	\begin{itemize}
		\item the \emph{centroid array}\todo{tady uz je emph zas zbytecnej} $\mathcal{C}$ as an array that contains centroids of all $k$ clusters,
...
\begin{defn}[Indexing structure]\todo{tohle neni definice, proste nahoru napis ze tu ctverici budes pojmenovavat `Indexing structure' a znacit Q. Btw je to jeden z mala pripadu kdy je emph povolen. :D Taky je mozny ze sem to naprosto spatne pochopil protoze ta definice je fakt nepruhledna a nedokazu rict co v tom Q teda vlastne ma bejt.}
	\xxx{Having} a dataset $\mathcal{D}$, its $k$ clusters and a corresponding cluster-based array tuple $(\mathcal{C},\mathcal{I},\mathcal{N},\mathcal{S})$, we define the \emph{indexing structure} $\mathcal{Q}$ that determines the order of the cluster-related data in these arrays.
	\label{def03:index}
\end{defn}
...
\todo{mozna tady chces jasne naznacit ze `The algorithm proceeds as follows'}
 If \texttt{apriori\_assignments} are present, \xxx{this} array is filled with clustering contexts. Each of them describes the data structure of the corresponding apriori cluster and holds methods for its further clustering. It will be further called the \emph{context array}. 

In addition to that, the method \texttt{run} uses one special clustering context called the \emph{final context}. If apriori clusters are present (and the context array is non-empty), \xxx{it} is unset and serves as a context that will host the apriori cluster results for the final clustering. If there are no apriori clusters, the final context is set and~contains the whole dataset.

We divide the whole clustering algorithm into the top level \emph{apriori clustering} and the low level \emph{context clustering}.\todo{Pochopeni by mozna pomohlo to nazvat layers, bez tyhle vety zbytek textu vypada ze to jsou 2 uplne oddeleny algoritmy}
...
\begin{description}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\While{$ctx$ \xxx{has not the only cluster}}
...
Each kernel group reads and modifies a different data array and has its own time complexity. Hence, to complete the kernel summary, we provide this information in the table \ref{tab03:kernels}. The following sections will describe each kernel group in more detail.
 ...
 \begin{table}
	...
	\renewcommand{\arraystretch}{1.5}
 	...
 	\end{tabular}
 	\caption{\xxx{The} summary of the kernel groups ($n$ is a number of points in a clustering context; $c$ is a number of clusters in a clustering context). \xxx{Trochu sem to predelal, zkontroluj to}}
 	\label{tab03:kernels}
 \end{table}
...
The kernel \texttt{merge\_clusters} is responsible for a merging of clusters. As the~input, it takes the~assignment array $\mathcal{A}$, a pair of old cluster IDs (of merged clusters) and a new ID (of a created cluster). It goes through the array in \xxx{the} \emph{grid-stride loop} and replaces any of the old IDs with the new ID.

\begin{defn}\todo{tadyto chce definovat poradne, technicky definice neni potreba, spis by byl vhodnej pseudokod nebo obrazek}
...
\todo{mozna by nebylo spatny tyhle vsechny popisky vysazet jako malinky algoritmy, aspon fakt hodne hodne pseudokodove. Ted to je dost neprehledny.}
...
First, the invalidated \xxx{places have to be recognized} by the kernel \texttt{update}. Next, two kernels \texttt{neighbors\_u} and \texttt{neighbors\_mat\_u} are run to update the~array --- the first one updates the Euclidean part while the second kernel updates the~Mahalanobis part. Then, the reduction kernel \texttt{reduce\_u} is called that reduces intermediate results generated by the previous two kernels. Last, the kernel \texttt{neighbors\_new} performs a special update of $\mathcal{N}$ for the newly created clusters.

In the apriori clustering, \xxx{an another kernel set is mentioned ---  \texttt{neighbors}}. \xxx{It} is used for the initialization of $\mathcal{N}$; \xxx{hence,} it is a modification of the currently described kernels. There, the \texttt{update} and \texttt{neighbors\_new} kernels are ignored as all elements of the array have to be updated and none of them is created. The~notation of kernel names suggests that kernels suffixed with \texttt{\_u} have a suffix-less variant that processes all elements.

\todo{Ty kernely je vhodny pojmenovat rozumne `lidsky', vzor nadpisu: `Update detection kernel', v textu nekde pak `This kernel is implemented in/called `update(.cu)'. Idealne stejne u vsech ostatnich kernelu.}
\subsubsection{Kernel \texttt{update}}
This kernel checks the validity of the neighbors of each cluster \xxx{via}\todo{via znamena `durch', chces `using'}\ a grid-stride loop. The check consists of these steps:
\begin{itemize}
...
\xxx{This kernel} takes care of updating clusters whose size have not reached the Mahalanobis threshold (we call them \emph{Euclidean clusters} for simplicity). For that reason, the input of the kernel is $\mathcal{C}$ (to compute distances) and $\mathcal{U}$ (to target clusters for the computation).

The first loop is not run in parallel, rather it is fully performed by all threads as it loops through $\mathcal{U}$\todo{Neni paralelni ale spoustej ho vsechny vlakna? wtf? Navic je trochu nejasny na jaky loopy se odkazujes. Ctenar si rozhodne automaticky neotevre vedle zdrojak a nebude do nej koukat (uz jen proto ze nevi kde).}. When cluster $c$ is selected to update, all threads contribute to the computation of its closest neighbors.

\xxx{There,} a grid-stride loop over other Euclidean clusters \xxx{follows}\todo{Takze je chvili ve stavu ze aktivne neco followuje?}. Each thread takes care of one cluster at a time, computes its distance with $c$ and updates its local closest neighbors of $c$.

Then, the block reduction follows\xxx{;}\todo{to je napojeny trochu podezrele}\ the block-local closest neighbors of $c$ are created. As there is no grid synchronization, each block stores its result in the~intermediate neighbor array $\mathcal{N}'$ ready for a further reduction.

When all clusters from $\mathcal{U}$ are visited, the kernel ends.\todo{obratit vetu}
...
\subsubsection{Kernel \texttt{neighbor\_mat\_u}} \xxx{This kernel shows similarities with} the previous kernel. The main distinction is that \xxx{it} updates clusters that reached the Mahalanobis threshold (\emph{Mahalanobis clusters}, for simplicity) instead of those that did not. We created the separate kernel \xxx{hoping to utilize the GPU more} and to achieve \xxx{a} better performance.\todo{vyslo to? Mozna ref do vysledku.}

In addition to $\mathcal{C}$ and $\mathcal{U}$, the kernel takes $\mathcal{I}$ as the input \xxx{as well}. The access to~the~inverse covariance matrices is \xxx{vital} as they are involved in a distance computation of Mahalanobis clusters. 

Same as in the previous kernel, the first loop is performed by all threads, then followed by a grid-stride loop. 

\xxx{However,} a \emph{warp}\todo{eeeeeemph, navic chces rict `each warp'}\ takes care of one cluster at a time in the grid-stride loop instead of a thread. Warp threads cooperate together in the matrix-vector multiplications as the Mahalanobis distance function is now involved. The previous kernel \xxx{need not to}~perform \xxx{such a} task for computing the distance; \xxx{hence,} only one thread is \xxx{assigned the job there}.

Last\todo{finally}, the kernel performs the same reduction as in the previous kernel and stores the block-local intermediate results into $\mathcal{N}'$.
