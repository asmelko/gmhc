...
\def\Abstract{%
In cytometry, researchers use the agglomerative hierarchical clustering with the Mahalanobis distance to cluster flow and mass cytometry datasets. The current implementation restricts the applicable size of clustering input to a factor of 100,000 cells. This thesis proposes the implementation accelerated on a single CUDA-enabled GPU. The improved performance and linear space complexity of the algorithm results in 5000x speedup for single-point datasets and 20x for apriori-datasets. With the accelerated algorithm, the applicable size of clustering input is increased to millions of cells.
}
