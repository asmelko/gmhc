...
\begin{algorithm}
\caption{demo jao vysazet algoritmus \xxx{odstranit}}
\label{alg:embedsom}
\begin{algorithmic}[1]
\Procedure{EmbedSOM}{$X\in\R^n$, $L\in\R^{n\times D}$, $l\in\R^{n\times 2}$, $k\in\{4\dots n\}$, $m>0$, $a>0$}
...
\For{$i'\in\{1\dots k-2\}$} \Comment iterate through pairs of $k-1$ closest landmarks
...
\begin{figure}\centering
	...
	\caption{An example of dendrogram. placeholder \xxx{tady k tomu sem pridal presnou velikost na sirku radku, aby to nesypalo ten osklivej overflow okraj}}
	\label{fig01:dendro}
\end{figure}
