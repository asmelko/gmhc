...
std::vector<pasgnd_t<float>> smhc::run()
{
    ...
    while (clusters_.size() > 1)
    {
        ...
        res.push_back(std::make_pair(std::make_pair(clusters_[min_pair[0]].id, clusters_[min_pair[1]].id), min_dist));
...
void smhc::merge_clusters(const std::array<size_t, 2>& cluster_pair)
{
    ...
    clusters_[cluster_pair[to]].add_points(
        clusters_[cluster_pair[from]].points.data(), clusters_[cluster_pair[from]].points.size() / point_dim);
