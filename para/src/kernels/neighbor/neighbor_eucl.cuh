    ...
    csize_t idx)
{
    ...
    for (csize_t y = threadIdx.x + blockIdx.x * blockDim.x + idx + 1; y < centroid_count; y += blockDim.x * gridDim.x)
    {
...
__global__ void neighbors(
    const float* __restrict__ centroids, csize_t dim, csize_t centroid_count, neighbor_t* __restrict__ neighbors)
{
